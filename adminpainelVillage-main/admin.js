...
function addBackToDashboardLink() {
    ...
    if (main) {
        ...
        viewLink.style.padding = '8px';
        viewLink.style.borderRadius = '6px';
        viewLink.style.background = 'var(--primary)';
        ...
        viewLink.onmouseover = () => viewLink.style.opacity = '0.8';
        viewLink.onmouseout = () => viewLink.style.opacity = '1';

        // O href será atualizado quando a config carregar
...
function setupTabs() {
  ...
  navItems.forEach(item => {
    item.addEventListener('click', () => {
      ...
      const q = document.querySelector('.topbar .search input')?.value?.toLowerCase() || '';
      applySearchFilter(q);
    });
  });
}
...
async function loadConfig() {
  try {
    // Buscar informações do projeto (nome e slug)
    ...
    if (project) {
      // Atualizar título da página com o nome do projeto
      document.title = `${project.name} - Admin Village`;
      
      // Atualizar link de visualizar site com o slug
      const viewLink = document.getElementById('view-site-link');
      if (viewLink) {
        viewLink.href = project.slug ? `/${project.slug}` : `view.html?id=${currentProjectId}`;
      }
    }
...
function setupPanelUX() {
  document.querySelectorAll('.config-card h4').forEach(h => {
  ...
  try {
    const saved = localStorage.getItem('adminCompact');
    if (saved === '1') document.body.classList.add('compact');
  } catch {}
  tagCardGroups();
  setupTabSubnav();
}
...
function applySearchFilter(q) {
  ...
  if (!activeTab) return;
  activeTab.querySelectorAll('.config-card').forEach(card => {
...
function tagCardGroups() {
  document.querySelectorAll('.config-card').forEach(card => {
    const title = card.querySelector('h4')?.textContent?.toLowerCase() || '';
    let group = '';
    if (title.includes('marca')) group = 'marca';
    else if (title.includes('fundos')) group = 'fundos';
    else if (title.includes('botões') || title.includes('botoes')) group = 'botoes';
    else if (title.includes('navegação') || title.includes('carrosséis')) group = 'navegacao';
    else if (title.includes('feedback')) group = 'feedback';
    else if (title.includes('texto promocional')) group = 'promocional';
    else if (title.includes('textos por seção')) group = 'textos';
    else if (title.includes('fontes') || title.includes('tamanhos')) group = 'fontes';
    else if (title.includes('estilo dos títulos')) group = 'titulos';
    else if (title.includes('hero') || title.includes('introdução')) group = 'hero';
    else if (title.includes('galeria')) group = 'galeria';
    else if (title.includes('agenda')) group = 'agenda';
    else if (title.includes('frames')) group = 'frames';
    if (group) card.dataset.group = group;
  });
}

function setupTabSubnav() {
  document.querySelectorAll('.tab-content').forEach(tab => {
    const cards = Array.from(tab.querySelectorAll('.config-card')).filter(c => c.dataset.group);
    if (!cards.length) return;
    const groups = [...new Set(cards.map(c => c.dataset.group))];
    ...
    allChip.addEventListener('click', () => {
      sub.querySelectorAll('.chip').forEach(ch => ch.classList.remove('active'));
      allChip.classList.add('active');
      cards.forEach(c => c.classList.remove('filter-hidden'));
    });
    sub.appendChild(allChip);
    groups.forEach(g => {
      ...
      chip.addEventListener('click', () => {
        ...
        cards.forEach(c => c.classList.toggle('filter-hidden', c.dataset.group !== g));
      });
      sub.appendChild(chip);
    });
    tab.insertBefore(sub, tab.firstChild);
  });
}
...
function updateConfigFromInput(input) {
  ...
  if (path) {
    ...
    if (input.classList.contains('color-hex')) {
      const isValid = typeof value === 'string' && /^#[0-9a-fA-F]{6}$/.test(value);
      if (!isValid) return;
    }
...
async function saveConfig() {
  ...
  try {
    ...
    try {
      localStorage.setItem('overrideConfig', JSON.stringify(currentConfig));
      showStatus('Preview local atualizado. Recarregue o site para ver as mudanças.', 'success');
    } catch {}
...
function setupTopbarActions() {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (!pop.contains(e.target) && !actionsEl.contains(e.target) && !(searchEl && searchEl.contains(e.target))) {
      pop.remove();
    }
  });
}
...
function setupPanelSearch() {
  ...
  const input = searchWrap ? searchWrap.querySelector('input') : null;
  if (!searchWrap || !input || input.dataset.bound === 'true') return;
  input.dataset.bound = 'true';

  const removeSearchPopover = () => {
    const existing = document.querySelector('.search-popover');
    if (existing) existing.remove();
  };

  const openTabById = (tabId) => {
    if (!tabId) return false;
    const nav = document.querySelector(`.nav-item[data-target="${String(tabId).replace(/"/g, '\\"')}"]`);
    if (nav) {
      nav.click();
      return true;
    }
  ...
  const collectResults = (term) => {
    ...
    navItems.forEach((btn) => {
      ...
      seen.add(key);
      results.push({ kind: 'tab', tabId, label, subtitle: 'Seção' });
    });

    const tabContents = Array.from(document.querySelectorAll('.tab-content[id]'));
    tabContents.forEach((tab) => {
      const tabId = tab.id;
      const tabTitle = (document.querySelector(`.nav-item[data-target="${String(tabId).replace(/"/g, '\\"')}"]`)?.textContent || tabId).trim();
      const nodes = Array.from(tab.querySelectorAll('h4, label, h3.section-title'));
      nodes.forEach((node) => {
        const text = (node.textContent || '').trim();
        if (!text) return;
        if (!normalizeSearchText(text).includes(term)) return;
        const key = `node:${tabId}:${text}`;
        if (seen.has(key)) return;
        seen.add(key);
        results.push({ kind: 'node', tabId, label: text, subtitle: tabTitle, node });
      });
    });
  ...
  const showPopover = (term) => {
    removeSearchPopover();
    const results = collectResults(term);

    const pop = document.createElement('div');
    pop.className = 'popover search-popover';

    const header = document.createElement('div');
    header.className = 'popover-header';
    header.textContent = results.length ? `Resultados (${results.length})` : 'Nenhum resultado';
    ...
    if (!results.length) {
      ...
      li.textContent = 'Nenhum item encontrado para esta busca';
      list.appendChild(li);
    } else {
      results.forEach((r, idx) => {
        ...
        icon.className = r.kind === 'tab' ? 'fa-solid fa-folder-open' : 'fa-solid fa-magnifying-glass';
        ...
        li.addEventListener('click', () => {
          removeSearchPopover();
          const opened = openTabById(r.tabId);
          if (opened && r.kind === 'node' && r.node) {
            window.setTimeout(() => {
              try {
    ...
    pop.style.width = `${Math.max(260, Math.round(rect.width))}px`;
  };
  ...
  input.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Enter') {
      const first = document.querySelector('.search-popover .popover-item[data-idx="0"]');
      if (first) {
        e.preventDefault();
        first.click();
      }
...
function showSettings() {
  ...
  compactItem.className = 'popover-item';
  const compactBtn = document.createElement('button');
  compactBtn.className = 'chip';
  compactBtn.textContent = document.body.classList.contains('compact') ? 'Modo compacto: ativo' : 'Modo compacto: desativado';
  compactBtn.addEventListener('click', () => {
    document.body.classList.toggle('compact');
    const enabled = document.body.classList.contains('compact');
    try { localStorage.setItem('adminCompact', enabled ? '1' : '0'); } catch {}
    compactBtn.textContent = enabled ? 'Modo compacto: ativo' : 'Modo compacto: desativado';
  });
  compactItem.innerHTML = '<i class="fa-solid fa-sliders"></i><span>Layout</span>';
  compactItem.appendChild(compactBtn);
  const clearItem = document.createElement('li');
  clearItem.className = 'popover-item';
  clearItem.innerHTML = '<i class="fa-solid fa-broom"></i><span>Limpar preview local</span>';
  clearItem.addEventListener('click', () => {
    try { localStorage.removeItem('overrideConfig'); showStatus('Preview local limpo', 'success'); } catch {}
  });
...
function playJingle() {
  try {
    ...
    const tone = (freq, t, dur, type = 'triangle', gain = 0.08) => {
    ...
    tone(659, now, 0.12);      // E5
    tone(784, now + 0.12, 0.12);
    tone(987, now + 0.24, 0.18);
    tone(1319, now + 0.42, 0.25); // E6
  } catch {}
}
...
function showQuickMenu() {
  ...
  const items = [
    ...
    { icon: 'fa-regular fa-eye', label: 'Abrir Site', action: () => { 
      if (currentProjectId) {
        const viewLink = document.getElementById('view-site-link');
        window.open(viewLink ? viewLink.href : `view.html?id=${currentProjectId}`, '_blank');
      }
    } },
