...
function renderCards(projectsToRender){
  ...
  projectsToRender && projectsToRender.forEach(project=>{
    ...
    card.innerHTML = `
        ...
        <a href="admin.html?id=${project.id}" class="card-title-wrap" style="color:inherit;text-decoration:none">
            <span class="card-title">${project.name || 'Projeto'}</span>
        </a>
    ...
    if (wrap && titleEl) {
      wrap.addEventListener('mouseenter', () => {
        ...
        const dist = Math.max(0, titleEl.scrollWidth - wrap.clientWidth);
        titleEl.style.setProperty('--title-distance', dist + 'px');
      });
      wrap.addEventListener('mouseleave', () => {
        titleEl.style.textOverflow = 'ellipsis';
        titleEl.style.removeProperty('--title-distance');
      });
    }
